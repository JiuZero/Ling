...
def _load_dark_qss(enable: bool):
    """
    在深色主题下加载 dark.qss，切换为浅色主题时清空样式。
    """
    try:
        ...
        if enable:
            ...
            if os.path.isfile(qss_path):
                with open(qss_path, "r", encoding="utf-8", errors="ignore") as f:
...
class SettingsInterface(QWidget):
    ...
    def load_settings(self):
        """加载 Ling 设置"""
        try:
            cfg_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ling_settings.json")
    ...
    def load_config(self):
        """加载 z0 的 config/config.py（位于 z0 工作目录）"""
        try:
            # 从 ling_settings.json 获取 z0 可执行路径
            z0_dir = None
            cfg_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ling_settings.json")
            if os.path.isfile(cfg_path):
                try:
                    with open(cfg_path, "r", encoding="utf-8") as f:
                        ...
                        if exe_path:
                            z0_dir = os.path.dirname(exe_path)
                except Exception:
                    z0_dir = None
            if not z0_dir:
                self.config_edit.setPlainText("# 未设置 z0 路径，请在设置页选择 z0.exe 或 z0.py")
                return
            cfg_py = os.path.join(z0_dir, "config", "config.py")
            if os.path.isfile(cfg_py):
                with open(cfg_py, "r", encoding="utf-8", errors="ignore") as f:
                    self.config_edit.setPlainText(f.read())
            else:
                self.config_edit.setPlainText("# 未找到 z0 工作目录下的 config/config.py")
        except Exception as e:
    ...
    def save_all(self):
        ...
        # 保存 z0 配置（写入 z0 工作目录的 config/config.py）
        try:
            z0_dir = None
            cfg_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ling_settings.json")
            if os.path.isfile(cfg_path):
                try:
                    ...
                    z0_dir = None
            if not z0_dir:
                InfoBar.error(title="错误", content="未设置 z0 路径，无法保存 z0 配置", position=InfoBarPosition.TOP, parent=self.parent_window or self, duration=3000)
                return
            cfg_py = os.path.join(z0_dir, "config", "config.py")
            os.makedirs(os.path.dirname(cfg_py), exist_ok=True)
...
class Z0ScanGUI(FluentWindow):
    ...
    def __init__(self):
        ...
        # 确定 z0 工作目录与 scanners 路径（基于 z0 可执行文件所在目录）
        self.z0_dir = None
        try:
            ling_cfg_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ling_settings.json")
            if os.path.isfile(ling_cfg_path):
                with open(ling_cfg_path, "r", encoding="utf-8") as _f:
                    _cfg = json.load(_f)
                    exe_p = (_cfg.get("exe_path") or "").strip()
                    if exe_p:
                        self.z0_dir = os.path.dirname(exe_p)
        except Exception:
            self.z0_dir = None
        self.scanner_dir = os.path.join(self.z0_dir, "scanners") if self.z0_dir else ""
        self.init_ui()
    ...
    def init_ui(self):
        ...
        try:
            ...
            if os.path.isfile(ling_cfg_path):
                with open(ling_cfg_path, "r", encoding="utf-8") as _f:
                    _cfg = json.load(_f)
                    is_dark = _cfg.get("theme", "LIGHT").upper() == "DARK"
                    setTheme(Theme.DARK if is_dark else Theme.LIGHT)
                    _load_dark_qss(is_dark)
        except Exception:
            pass
        ...
        # 设置页（合并到本文件）
        self.settings_page = SettingsInterface(self)
    ...
    def create_scan_tab(self):
        ...
        advanced_layout.addLayout(cols_layout)
    ...
    def load_plugins(self):
        """从 z0 工作目录的 scanners/ 下加载 PerPage、PerDir、PerDomain、PerHost 插件列表"""
        # 依据 ling_settings.json 实时刷新 z0_dir 与 scanners 根路径
        try:
            ling_cfg_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ling_settings.json")
            if os.path.isfile(ling_cfg_path):
                with open(ling_cfg_path, "r", encoding="utf-8") as _f:
                    ...
                    if exe_p:
                        self.z0_dir = os.path.dirname(exe_p)
                        self.scanner_dir = os.path.join(self.z0_dir, "scanners")
        except Exception:
            pass
    ...
    def start_scan(self):
        ...
        # 优先使用设置页指定的 z0 路径；支持 .exe 与 .py
        command = None
        python_cmd = "python" if sys.platform.startswith('win') else "python3"

        if exe_override and os.path.isfile(exe_override):
            if exe_override.lower().endswith(".py"):
                command = f'{python_cmd} "{exe_override}" scan'
            else:
                command = f'"{exe_override}" scan'

        # 回退到当前目录
        if command is None:
            exe_path = os.path.join(current_dir, default_name)
            if os.path.isfile(exe_path):
                command = f'"{exe_path}" scan'
            else:
                z0_py = os.path.join(current_dir, "z0.py")
                command = f'{python_cmd} "{z0_py}" scan'
        ...
        try:
            if exe_override and os.path.isfile(exe_override):
                run_cwd = os.path.dirname(exe_override)
        except Exception:
            pass
        self.scan_thread = ScanThread(command, cwd=run_cwd)
    ...
    def ensure_exe_path_set(self):
        """
        启动后校验是否已设置 z0 可执行文件路径；若未设置或无效，则强制提示用户选择。
        """
        try:
            cfg_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ling_settings.json")
            ...
            def _is_valid(path):
                try:
                    return bool(path) and os.path.isfile(path) and (path.endswith(".exe") or path.endswith(".py"))
                except Exception:
                    return False

            if _is_valid(exe_path):
                return

            while True:
                fname, _ = QFileDialog.getOpenFileName(self, "请选择 z0 可执行文件（z0.exe 或 z0.py）", os.getcwd(), "可执行或脚本 (*.exe *.py)")
                if not _is_valid(fname):
                    reply = QMessageBox.question(self, "必须设置 z0 路径", "未设置 z0 路径将无法使用扫描功能。\n是否退出程序？", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
                ...
                # 保存设置
                try:
                    new_data = {"exe_path": fname, "theme": "LIGHT"}
                    if os.path.isfile(cfg_path):
                        try:
                            with open(cfg_path, "r", encoding="utf-8") as f:
                                old = json.load(f)
                    ...
                    try:
                        if getattr(self, "settings_page", None):
                            self.settings_page.exe_input.setText(fname)
                    except Exception:
                        pass
                    try:
                        InfoBar.success(title="成功", content=f"已设置 z0 路径: {fname}", position=InfoBarPosition.TOP, parent=self, duration=3000)
                    except Exception:
                        pass
                except Exception as e:
                    try:
                        InfoBar.error(title="错误", content=f"保存设置失败: {e}", position=InfoBarPosition.TOP, parent=self, duration=4000)
                    except Exception:
                        pass
                break
        except Exception:
...
def ensure_ling_exe_path(parent=None):
    """
    在进入主界面前确保已设置 z0 可执行路径；未设置或无效则循环提示选择，用户可选择退出。
    返回 True 表示已就绪，False 表示用户选择退出。
    """
    try:
        cfg_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ling_settings.json")
        ...
        # 循环要求用户选择
        while True:
            fname, _ = QFileDialog.getOpenFileName(parent, "请选择 z0 可执行文件（z0.exe 或 z0.py）", os.getcwd(), "可执行或脚本 (*.exe *.py)")
            if not _is_valid(fname):
                reply = QMessageBox.question(parent, "必须设置 z0 路径", "未设置 z0 路径将无法使用扫描功能。\n是否退出程序？", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
                if reply == QMessageBox.Yes:
                    return False
                else:
                    continue
